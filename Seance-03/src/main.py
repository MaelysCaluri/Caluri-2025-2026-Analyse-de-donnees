...
for i in range(len(colonnes)):
    if types_colonnes[i] in [float, int]: # si la colonne est quantitative :
        ...
        eca_abs_mad = new_col.mad()
        ecarts_absolus_mad.append(round(float(eca_abs_mad), 2))

        # On calcule et stocke l'écart absolu à la moyenne, des valeurs de la colonne
        #   La méthode .mad existe (pas sur toutes les versions), on calcule donc manuellement :
...
print("ecarts_absolus_mad =", ecarts_absolus_mad)
